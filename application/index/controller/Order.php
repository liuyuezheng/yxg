...
//                dump($where2);
        ...
        if(isset($res['nameorder']) && !empty($res['nameorder'])){
            $where3['yx_orders.telephone|yx_orders.orders_num'] = ['like',"%{$res['nameorder']}%"];
...
//            exit();
            ...
            if(isset($attrAssment2) && !empty($attrAssment2)){
                $price->where('id',(int)$attr)->update($attrAssment2);
            }
            if(isset($attrAssment) && !empty($attrAssment)){
                $price->where('id',$attr_id)->update($attrAssment);
            }
...
//        积分
        ...
        //会员信息
        $users=$user->where('id',$orders['user_id'])->field('user_num,telephone')->find();
        //会员上级分佣
...
//      佣金分配信息
        ...
        $expectBros=$expects->alias('e')
            ...
            ->field('e.grade,e.money,u.nickname,u.telephone')
            ->select();
        $arr1=[];
        $arr2=[];
        $arr3=[];
...
//                        $shuju[$j]['telephone'] = $v[4];
                            ...
                            $shuju2[$j]['logistics']=trim($v[1]);
//                        $shuju2[$j]['order_id']=(int)trim($v[0]);
                            $shuju2[$j]['logistics_num']= trim((string)$v[2]);
                            //发货
                            $shuju2[$j]['update_time']=time();
                            if(!empty(trim((string)$v[1])) && !empty(trim((string)$v[2]))){
