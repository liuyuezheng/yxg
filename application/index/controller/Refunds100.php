...
//        dump($where3);
        /*   (isset($res['nameorder']) && !empty($res['nameorder'])) ? $where4['d.goods_name|r.refund_num'] = ['like',"%{$res['nameorder']}%"]:$where4=[];*/
...
//          余额
                    ...
                    $refund_status=$pay->insert(['user_id'=>$users['id'],'title'=>'退款到账','money'=>$refunds['refund_money'],'type'=>0,'status'=>0,'time'=>time()]);
                }else{
...
//        refund_reject
        $refund=new Refund();
        $detail=new OrdersDetail();
        $refunds=$refund->where('id',$id)->field('order_id')->find();
//            更新小订单表状态
        $detail->where('id',$refunds['order_id'])->update(['status'=>4,'update_time'=>time()]);
