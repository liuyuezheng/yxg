...
class Orders extends Model
{
   ...
   public static function promptly_goods($id = 0,$attr_id = 0,$count = 0,$goods_specification = '',$address_id = 0){
       ...
       $data['goodsinfo'][] = $list;
       $data['goods_total_price'] = $goods['price'] * $count+$freight_price;
       return $data;
   ...
   public static function create_orders($data){
      ...
      if($data['coupon_id'] == 0){
        ...
        Db::table('yx_my_coupon')->where(['id'=>$coupon_id])->update(['status'=>1]);
         ...
         $address = Db::table('yx_address')->where('user_id',$data['user_id'])->find();
         if(empty($address)){
             return 3;
         } 
         //dump($list);die;
      if($data['cart_id'] != ''){
          ...
          foreach ($cart_id as $k => $v) {
              ...
              if($attrinfo['num'] >= $cartinfo['count']){
                 ...
                 //        dump($goods_price);
                 // dump($goods_total_price);  
                 // dump($goods_price/$goods_total_price*$coupon);
                  //dump($ordergoods);//die;
           ...
           if($attrinfo['num'] >= $data['count']){
             ...
             if($order_id > 0){
                 ...
                 $ordergoods['goods_price'] = $attrinfo['price'];
    ...
    * 
    */
   public static function ordersinfo($id = 0,$orders_num = '',$type = 0){
        $where = [
            'orders_num'=>$orders_num
        ];
        ...
        if($ordersinfo['orders_total_price'] <= $userinfo['balance']){
            if(Db::table('yx_orders')->where($where)->update($data) === false){
             ...
             foreach ($order_detail as $k => $v) {
                 ...
                 if(!empty($one_agent)){
                    $aa['money'] = $goods_assessment['one_agent'] * $v['goods_count']; //查询一级总代所分佣金
                 ...
                 if(!empty($one_manager)){
                     //一级董事的所分佣金
                    $aa4['money'] = ($goods_assessment['price'] - $goods_assessment['cost_price']) * $v['goods_count'];
   ...
   public static function orders_list($id = 0,$status = 0,$page = 1){
     ...
     //dump($where);die;
      ...
      ->limit($start,$num)->order('yo.create_time desc')->select();
      ...
      foreach ($data['orders'] as $k => $v) {
          ...
          $data['orders'][$k]['order_detail'] = Db::table('yx_orders_detail')->alias('yod')->where($where1)->join($join)
          ->field($field)->select();
          if($v['status'] == 0){
            if(($v['create_time'] + (30 * 60)) < time()){
             Db::table('yx_orders')->where(['id'=>$v['b_id']])->update(['status'=>4]);
            }
          }
   ...
   public static function orders_detail($id = 0){
       ...
       $field = [
           'id as b_id,orders_num,address,name,telephone,coupon,freight_price,status,orders_total_price,remark,create_time'
       ];
   ...
   public static function orders_arrive($id = 0){
      ...
      if(Db::table('yx_orders')->where($where)->update($data) === false){
         ...
         foreach ($order_detail as $k => $v) {
             ...
             Db::table('yx_orders_detail')->where('id',$v['id'])->update(['status'=>0]);
         }
         ...
         //dump($user_brokerage);die;
          ...
          if($user){
             ...
             update(['integral'=>$user['integral']+$own_integral]);
             //添加到一级用户积分明细
             $list5 = [
                'user_id'=>$user['id'],
                'title'=>'购买商品',
                'integral'=>$own_integral,
                'time'=>time()
             ];
         ...
         if($one_integral){
             ...
             update(['integral'=>$one_integral['integral'] + $one_pid_integral]);
             //添加到一级用户积分明细
             $list6 = [
         ...
         if($two_integral){
             ...
             update(['integral'=>$two_integral['integral'] + $two_pid_integral]);
             //添加到一级用户积分明细
             $list7 = [
         ...
         if($three_integral){
             ...
             update(['integral'=>$three_integral['integral'] + $three_pid_integral]);
              //添加到三级用户积分明细
             $list8 = [
...
//                    董事
   ...
   public static function refund_order($id = 0,$page = 1){
       ...
       $field = [
           'yod.id,yod.goods_image,yod.goods_name,yod.goods_specification,yod.goods_count,yr.type,yr.refund_status'
       ];
       $join = [
   ...
   public static function refund_detail($id = 0){
       ...
       $field = [
           ...
           yr.refund_reason,yr.refund_intro,yr.refund_reject,yr.update_time'
       ];
       $join = [
           ['yx_refund yr','yr.order_id = yod.id']
       ];
    ...
    * 
    */
   public static function tracking_num($id = 0,$tracking_num = 0){
       $where = [
           'id'=>$id
       ];
       $update = [
   ...
   public static function comment_orders($id = 0){
       ...
       $data = Db::table('yx_orders_detail')->field($field)->where('order_id',$id)->select();
       //dump($data);die;
       return $data;
   }
