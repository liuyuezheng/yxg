...
$(function(){
	...
	$(document).on("change", ".cusSkuTypeInput", function(){
		...
		$(this).parent().attr("sku-type-name",$(this).val());   //一级属性存在时，就赋值给对应的ul
	...
	$(document).on("change",".model_sku_val",function(){
		...
		//SKU类型
		var skuTypeVal = $(this).parent().parent().prev().find("li").attr("sku-type-name");   //一级栏目 
//		console.log('skuTypeVal:'+skuTypeVal);
		//是否设置了sku类型及sku值
...
//				
				...
				if(alreadySetSkuVals){
					...
					if(currGroupSkuVal){
						...
						timePrice=currGroupSkuVal.timePrice;
...
SKUTableDom += '<tr propvalids=\''+propvalids+'\' propids=\''+propIdArr.toString()+'\' propvalnames=\''+propvalnameArr.join(";")+'\'  propnames=\''+propNameArr.join(";")+'\' class="sku_table_tr">'+currRowDoms+
				'<td><input type="file" onchange="showImgview(this)" data-img="" multiple class="setting_img" value="'+proImg+'"/></td>'+
				...
				// '<td><input type="text" class="setting_time_price" value="'+timePrice+'"/></td>'+
...
function getAlreadySetSkuVals(){
	...
	$("tr[class*='sku_table_tr']").each(function(){
		var proImg=$(this).find("input[type='text'][class*='setting_img']").val();                  //图片
		...
		var timePrice= $(this).find("input[type='text'][class*='setting_time_price']").val(); //折扣价
...
$(".getSetSkuVal").on("click",function(){
	...
	$("tr[class*='sku_table_tr']").each(function(){
		var obj={propnames:'',propvalnames:'',proImg:'',originPirce:'',vipPrice:'',timePrice:'',storeCount:''};
		obj.proImg=$(this).find("input[type='text'][class*='setting_img']").val();        //图片
		obj.originPirce = $(this).find("input[type='text'][class*='setting_origin_price']").val(); //原价
		obj.vipPrice = $(this).find("input[type='text'][class*='setting_vip_price']").val();  //会员价
		obj.timePrice= $(this).find("input[type='text'][class*='setting_time_price']").val();
		obj.storeCount = $(this).find("input[type='text'][class*='setting_sku_stock']").val();//SKU库存
