...
//	    dump($_SERVER['DOCUMENT_ROOT'].'/cert'.DIRECTORY_SEPARATOR.'apiclient_cert.pem');
//	    die;
        //http://www.oicqzone.com/pc/2018041324145.html
        //dump($_SERVER['DOCUMENT_ROOT'].'/cert'.DIRECTORY_SEPARATOR.'apiclient_cert.pem');die;
//        $where = [
//            'wr.is_return'=>0,
//            'wq.technician_id'=>['neq',0]
//        ];
//        $field = [
//            'wr.id as w_id,wq.id as qw_id,wr.order_num,wr.money,wq.technician_id,wr.time'
//        ];
//        $data = Db::table('wc_recharge')->alias('wr')->field($field)->join('wc_question wq','wq.out_trade_no=wr.order_num')->where($where)->select();
////dump($data);die;
//        foreach ($data as $k => $v){
            // dump($v);
//            $arr = Db::table('wc_question_answer')->where('question_id',$v['qw_id'])->find();
//            if(empty($arr) && (time()>($v['time']+10))){
        ...
        die;
//                if($result){
//                    Db::table('wc_recharge')->where('id',$v['w_id'])->update(['is_return'=>1]);
//                    $list = Db::table('wc_userinfo')->where('id',$v['technician_id'])->find();
//                    Db::table('wc_userinfo')->where('id',$v['technician_id'])->update(['balance'=>$list['balance']-$v['money']]);
//                }
