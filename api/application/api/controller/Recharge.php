	...
	if ($result['result_code']=='SUCCESS' && $result['return_code']=='SUCCESS') {
       ...
       $where = [
        ...
        $data = [
         ...
         Db::table('yx_recharge')->where($where)->update($data);//修改订单状态
         $data = Db::table('yx_recharge')->where($where)->find();
         $user = Db::table('yx_user')->where(['id'=>$data['user_id']])->find();
         file_put_contents('test1111.txt',$user);
         Db::table('yx_user')->where(['id'=>$data['user_id']])->update(['balance'=>$user['balance']+$data['money']]);
         $list = [
...
public function pay(){ 
     ...
     $notify_url = 'https://yxg.mumarenkj.com/api/recharge/callback'; //回调的url【自己填写】 
     $openid = $openid; 
     $out_trade_no = rand(100,9999).time();//商户订单号 
