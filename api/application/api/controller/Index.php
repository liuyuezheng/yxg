...
class Index extends Controller{
     ...
     * 
     */
    public function goods_attribute($id = 0,$user_id = 0){
        $data = index_model::goods_attribute($id,$user_id);
        if($data){
     ...
     * 
     */
    public function automatic_goods(){
      $where = [
          'start_time'=>['gt',0],
          ''
      ];
      $data = Db::table('yx_goods')->select();
    }
    ...
    public function putaway_goods(){
       ...
       foreach ($data as $k => $v) {
          ...
          if($v['start_time'] < time() && $v['end_time'] > time()){
             Db::table('yx_goods')->where(['id'=>$v['id']])->update(['is_putaway'=>0]);
          }
       }
    ...
    public function outof_goods(){
       ...
       foreach ($data as $k => $v) {
          if($v['end_time'] < time()){
             Db::table('yx_goods')->where(['id'=>$v['id']])->update(['is_putaway'=>1]);
          }
       }
      // Db::table('yx_redis_cs')->insert(['user_id'=>22]);
    }
