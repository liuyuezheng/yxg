<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:59:"F:\ayxg\public/../application/index\view\kitings\index.html";i:1552468266;}*/ ?>
<!DOCTYPE html>
<html>
        ...
        <div class="col-sm-12">
            <div class="ibox float-e-margins">
                <div class="ibox-title">
                    用户提现&nbsp;&nbsp;<img style="width: 20px" onclick="history.go(0)" src="/static/hplus/img/123.jpg">
                </div>
 ...
 -->
                    <div class="input-group" style=" width: 100%;">
                       ...
                       <!--     <input type="text" class="layui-input" style="height: 23px;width: 165%;" id="test5" placeholder="申请日期">-->
                        ...
                        <a data-toggle="modal" class="btn btn-w-m btn-danger" id="getAllSelectedId" style="margin-left: 10px"  href="form_basic.html#modal-form">提现记录</a>
                    ...
                    <table class="table table-striped table-bordered table-hover dataTables-example">
                        ...
                        <tr style="text-align: center;">
                            ...
                            <th style="text-align: center;">提现去向</th>
                        ...
                        <tr class="gradeX">
                            <td><?php if(($v['status'] == 0)): ?>
                                <input type="checkbox" data-status="<?php echo $v['type1']; ?>" data-status2="<?php echo $v['type2']; ?>" name = "items" id = "<?php echo $v['id']; ?>" />
                            ...
                            <td><?php if($v['type2'] == '0'): ?>余额<?php endif; if($v['type2'] == '1'): ?>佣金<?php endif; ?>
                            </td>
                            ...
                            <td class="center">
                                ...
                                <a class="btn btn-primary btn-rounded" style=" border-radius: 4px"   onclick="update_showorder(this)" data-status="<?php echo $v['type1']; ?>" data-id="<?php echo $v['id']; ?>" data-status2="<?php echo $v['type2']; ?>">通过</a>
                                <a class="btn btn-primary btn-rounded" style=" border-radius: 4px"  onclick="update_slidedel(this)" data-status="1" data-id="<?php echo $v['id']; ?>">拒绝</a>
                                <?php elseif(($v['status'] == 1)): ?>
                                <a class="btn btn-primary btn-rounded" style=" border-radius: 4px" >已通过</a>
                                <?php else: ?>
                                <a class="btn btn-primary btn-rounded" style=" border-radius: 4px"   >已驳回</a>
                                <?php endif; ?>
            ...
            <a data-toggle="modal" class="btn btn-w-m btn-danger" id="getAllSelectedId3"  href="form_basic.html#modal-form" style="margin-top: -64px;">批量通过</a>
            <a data-toggle="modal" class="btn btn-w-m btn-danger" id="getAllSelectedId4"  href="form_basic.html#modal-form" style="margin-top: -64px;">批量拒绝</a>
        </div>
    </div>
</div>
...
<script>
    ...
    function update_showorder(update_showorder){
        ...
        var status = $(update_showorder).attr('data-status');
        var status2 = $(update_showorder).attr('data-status2');
        var str="";
        if(status==0){
            str="是否已打款";
        }else{
        ...
        layer.msg(str, {
            ...
            ,yes: function(index){
                layer.close(index);
                $.ajax({
                    url: "<?php echo url('kitings/agrees'); ?>",
                    type: "post",
                    data: {
                        id:id,type1:status,type2:status2
                    },
                    dataType: "json",
...
//                            window.location.href = "<?php echo url('kitings/index'); ?>"
    ...
    function member_out(member_out) {
        ...
        if(time1==''){
            window.location.href = "<?php echo url('kitings/upexcel'); ?>?time="+time1+"&nameorder="+nameorder;
        }else{
            if(reg1.test(time1)){
                window.location.href = "<?php echo url('kitings/upexcel'); ?>?time="+time1+"&nameorder="+nameorder;
            }else{
...
//        alert(str);
        layer.ready(function(){
            layer.open({
                ...
                maxmin: true,
                area: ['50%', '40%'],
                content: '<?php echo url('wherereject'); ?>?id='+id,
    ...
    $('#getAllSelectedId3').click(function(){
        $("input[name='items']:checked").each(function(){
            ...
            status3 = $(this).attr('data-status');
            status4 = $(this).attr('data-status2');
        });
        console.log("id长度"+ids.length);
        if(ids.length == 0){
            layer.msg('请选择提现订单', {icon: 2, shift: 6});
            return;
        }
//        main
        layer.msg("是否批量通过", {
            ...
            ,yes: function(index){
                layer.close(index);
                $.ajax({
                    url: "<?php echo url('kitings/allagree'); ?>",
                    type: "post",
                    data: {
                        ids:ids,type1:status3,type2:status4
                    },
                    dataType: "json",
                    success: function (data) {
                        console.log(data);
...
//        main
        layer.msg("是否批量拒绝", {
            ...
            ,yes: function(index){
                layer.close(index);
                $.ajax({
                    url: "<?php echo url('kitings/allreject'); ?>",
                    type: "post",
                    data: {
                        ids:ids2,type:2
                    },
...
//                        }
    ...
    $('#getAllSelectedId').click(function(){
      ...
      window.location.href = "<?php echo url('kitings/agree'); ?>"
